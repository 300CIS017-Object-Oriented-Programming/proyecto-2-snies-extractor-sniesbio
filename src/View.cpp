...
// Se cargan las rutas desde un archivo de configuración
View::View()
{
...
std::string View::pedirAno(const std::string& mensaje)
{
    std::string anio;
    int intentos = 0;
    while (!esConvertibleAEntero(anio))
    {
        if (intentos > 0)
        {
            std::cout << "El valor ingresado no es válido. Por favor, ingrese un año válido." << std::endl;
        }
        std::cout << mensaje;
        std::cin >> anio;
        intentos++;
